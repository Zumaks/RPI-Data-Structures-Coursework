...
std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    hash_list_itr itr = std::find(m_table[index].begin(), m_table[index].end(), key);
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::swap(table_size, new_table.table_size);
    std::swap(num_elements, new_table.num_elements);
  }
};
#endif