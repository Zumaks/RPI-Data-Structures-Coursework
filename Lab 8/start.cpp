...
int pathwaycounter(int startingx, int startingy, std::vector<std::vector <int> > blocked_points) { 
    ...
    if (startingx == 0 && startingy == 0) { 
        ...
        int pathways = 0;
        pathways = pathways + pathwaycounter(startingx,startingy - 1,blocked_points);
        pathways = pathways + pathwaycounter(startingx-1,startingy,blocked_points);
        return pathways;
    }   
}
...
int main(int argc, char* argv[]) {
  ...
  for(int i = 0; i < blocked_grid[0].size(); i++) {
    ...
    int pathnum = pathwaycounter(start_x,start_y,blocked_coords);
