...
// int pathwaycounter(int startingx, int startingy, std::vector<std::vector <int> > blocked_points,std::vector < std::vector <int> > &vec) { 
//     for(int i = 0; i < blocked_points.size(); i++) {
//         if (startingx == blocked_points[i][0] && startingy == blocked_points[i][1]) {
//             vec.pop_back();
//             return 0;
//         }
//     }
...
//         temp_vec.push_back(startingx);
//         temp_vec.push_back(startingy);
//         vec.push_back(temp_vec);
//         temp_vec.clear();
//         pathways = pathways + pathwaycounter(startingx,startingy - 1,blocked_points,vec);
//         pathways = pathways + pathwaycounter(startingx-1,startingy,blocked_points,vec);
//         return pathways;
//     }   
// }
...
//   std::vector< std::vector < int> > blocked_coords;
//   for(int i = 0; i < blocked_grid[0].size(); i++) {
//         for(int z = 0; z < blocked_grid.size();z++) {
//             if (blocked_grid[z][i] == GRID_BLOCKED) { 
//                 std::vector <int> temp_vec;
//                 temp_vec.push_back(z);
//                 temp_vec.push_back(i);
//                 blocked_coords.push_back(temp_vec);
//                 temp_vec.clear();
//             }
//         }
//   }
//     std:: vector < std::vector <int> > temp_vec;
//     int pathnum = pathwaycounter(start_x,start_y,blocked_coords,temp_vec);
//     std::cout << pathnum << std::endl;

//     for(int i =0; i < temp_vec.size(); i++) {
//         std::cout << temp_vec[i][0] << " " << temp_vec[i][1] << std::endl;
//     }
...
void find_paths(int x, int y, std::vector<std::vector<GRID_STATUS> >& blocked_grid, int& path_count) {
  ...
  if (x == 0 && y == 0) {
    ...
    for (int j = 0; j < blocked_grid[0].size(); j++) {
      for (int i = 0; i < blocked_grid.size(); i++) {
        if (blocked_grid[i][j] == GRID_PATH)
std::cout << " $";
else if (i == x && j == y)
std::cout << " S";
else if (blocked_grid[i][j] == GRID_BLOCKED)
std::cout << " X";
else
std::cout << " .";
}
...
blocked_grid[x][y] = GRID_PATH;
find_paths(x - 1, y, blocked_grid, path_count);
find_paths(x, y - 1, blocked_grid, path_count);
blocked_grid[x][y] = GRID_CLEAR;
}

void find_paths(std::vector<std::vector<GRID_STATUS> >& blocked_grid, int start_x, int start_y) {
int path_count = 0;
find_paths(start_x, start_y, blocked_grid, path_count);
if (path_count == 0) {
std::cout << "No paths found\n";
} else {
std::cout << "Found " << path_count << " path(s)\n";
}
}
...
find_paths(blocked_grid, start_x, start_y);
